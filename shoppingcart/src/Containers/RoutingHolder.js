import React , {Component} from 'react';
import {Router , Route } from 'react-router-dom';
import Frontpage from './Fronpage';
import Productlist from './Productlist';
import CurrentCartList from './CurrentCartList';
import Login from './Login';
import Singnup from './Singnup';


                

class RoutingHolder extends Component {


    render(){
        return(
                <div>
                    <Router history={this.props.history}>
                    <Route path="/" component={Frontpage} exact={true}/> 
                        <Route path="/Loginpage" component={(props)=> <Login {...props} history={this.props.history} />} exact={true}/> 
                        <Route path="/products" component={ (props) => <Productlist {...props}count={this.props.count}/>}  />
                        <Route path="/currentcart" component={ (props) => <CurrentCartList {...props}count={this.props.count}/>}  />
                        <Route path="/Signup" component={Singnup} />
                    </Router>
                </div>

        )
    }
}

export default RoutingHolder;